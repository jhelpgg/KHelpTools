                                   ...
                                   length : Int, channels : Int, sampleSize : Int) : Sound()
{
                ...
                when (nbChannels)
                {
                    1    ->
                        when (sampleSize)
                        {
                            ...
                            else -> throw SoundException("Only sample size of 8 or 16 are managed, not ${sampleSize}")
                        }
                    2    ->
                        when (sampleSize)
                        {
                    ...
                    else -> throw SoundException("Only mono or stereo are managed, nbChannels=${nbChannels}")
                }
     ...
     */
    override fun transferToBuffer(buffer : Int) : Boolean
    {
        if (! this.valid)
        {
            return false
