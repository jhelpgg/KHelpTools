...
class SoundMP3(inputStream : InputStream) : Sound()
{
        ...
        try
        {
            ...
            this.format =
                when (channels)
                {
                    ...
                    else -> throw SoundException("Only mono OR stereo are supported, not: channels=$channels")
                }
     ...
     * @return `true` if their more frame to read. `false` if their no more frame to read
