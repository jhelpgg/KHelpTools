...
import khelp.image.gif.computeGifSize
import khelp.image.pcx.computePcxSize
import khelp.thread.TaskContext
import khelp.thread.parallel
import khelp.utilities.collections.EnumerationIterator
...
class FileImageInformation(val file : File)
{
    ...
    override fun toString() =
        "${this.file.absolutePath} : ${this.width}x${this.height} ${this.formatName}"
    ...
    companion object
    {
        ...
        init
        {
            ...
            {
                val future = parallel(TaskContext.MAIN) { ImageIO.getImageReadersBySuffix(suffix) }

                for (imageReader in EnumerationIterator(future.result()))
                {
