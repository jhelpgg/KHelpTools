package khelp.image.transformation

import khelp.utilities.math.isNul
import khelp.utilities.math.square

/**
 * Transformation on image pixels, it translates some pixels to a different place
...
class Transformation(val width : Int, val height : Int)
{
    ...
    private fun check(x : Int, y : Int)
    {
        ...
        {
            throw IllegalArgumentException("($x,$y) is outside the transformation ${this.width}x${this.height}")
        }
    }

    /**
     * Combine actual transformation with horizontal sinusoid
     *
    ...
    /**
     * Combine actual transformation with vertical sinusoid
     *
     * @param numberOfWave Number of wave
     * @param amplitude    Wave size
     * @param angleStart   Start angle in radiant
