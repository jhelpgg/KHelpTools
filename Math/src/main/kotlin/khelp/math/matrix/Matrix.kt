...
class Matrix(val width: Int, val height: Int)
{
    ...
    fun multiplication(matrix: Matrix): FutureResult<Matrix>
    {
        ...
        for (y in 0 until this.height)
        {
            ...
            {
                futures += parallel(TaskContext.INDEPENDENT,
                                    TaskMultiplicationCell(x, y, this.width, matrix.width, matrix.width, result.matrix,
                                                           this.matrix, matrix.matrix),
                                    this.taskMultiplicationCell)
            }
        }
