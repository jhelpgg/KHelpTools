...
class FutureResult<R : Any> internal constructor(val taskContext : TaskContext)
{
     ...
     * @return Future on the combination of this future followed by given task
     */
    fun <R1 : Any> andUnwrap(context : TaskContext = this.taskContext,
                             task : (R) -> FutureResult<R1>) : FutureResult<R1>
    {
    ...
    fun result() : R
    {
        ...
        when (this.status.get())
        {
            FutureResultStatus.CANCELED -> throw this.cancellationException
            FutureResultStatus.FAILED   -> throw this.failureException
